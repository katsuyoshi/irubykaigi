...
@tmpfile = @store_dir + "timetables.tmp"
...
# 一回書き出してパースしないと順番が同じにならない？
File.open(@tmpfile, 'w') do |f|
  f.write WebAPI::JsonBuilder.new.build(timetables)
end
new_timetables = WebAPI::JsonParser.new.parse(File.read(@tmpfile))

old_timetables = File.exists?(@filename) ? WebAPI::JsonParser.new.parse(File.read(@filename)) : {}
unless new_timetables['ja'] == old_timetables['ja'] && new_timetables['en'] == old_timetables['en']
